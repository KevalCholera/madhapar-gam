...
public class ProfileUpdateModel implements ProfileUpdateModelInt {
    ...
    public void createNewLocation(final String locationName, final ProfileUpdateListener mProfileUpdateListener) {
            ...
            public void onResponse(String response) {
                if (response != null) {
                    try {
                        ...
                        if (responseObj.optInt("status") == Constants.ResponseCode.SignUpSuccessCode) {
                            mProfileUpdateListener.onSuccessUpdateProfile(responseObj.optString("message"));
                        } else {
                            mProfileUpdateListener.onFailProfileUpdateRequest(responseObj.optString("message"));
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
        ...
        createLocationRequest.setRetryPolicy(new DefaultRetryPolicy(UtilClass.RetryTimeOut,
                DefaultRetryPolicy.DEFAULT_MAX_RETRIES,
                DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        MadhaparGamApp.getAppInstance().

                addToRequestQueue(createLocationRequest, tag);
    }
    ...
    private void updateInformation(JSONObject userData) {
        ...
        SharedPreferenceUtil.putValue(Constants.UserData.UserFamilyMemberCount, userData.optString("userFamilyMemberCount"));
